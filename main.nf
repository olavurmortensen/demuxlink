...
process merge {
    ...
    mkdir outs
    zcat $sample\\_*L001\\_R1*.fastq.gz | gzip -c > outs/$sample\\_L001\\_R1.fastq.gz
    zcat $sample\\_*L002\\_R1*.fastq.gz | gzip -c > outs/$sample\\_L002\\_R1.fastq.gz
    zcat $sample\\_*L003\\_R1*.fastq.gz | gzip -c > outs/$sample\\_L003\\_R1.fastq.gz
    zcat $sample\\_*L004\\_R1*.fastq.gz | gzip -c > outs/$sample\\_L004\\_R1.fastq.gz
    zcat $sample\\_*L001\\_R2*.fastq.gz | gzip -c > outs/$sample\\_L001\\_R2.fastq.gz
    zcat $sample\\_*L002\\_R2*.fastq.gz | gzip -c > outs/$sample\\_L002\\_R2.fastq.gz
    zcat $sample\\_*L003\\_R2*.fastq.gz | gzip -c > outs/$sample\\_L003\\_R2.fastq.gz
    zcat $sample\\_*L004\\_R2*.fastq.gz | gzip -c > outs/$sample\\_L004\\_R2.fastq.gz
    # TODO: remove later
    # This is for testing, running on the tiny-bcl data.
